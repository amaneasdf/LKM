...
Public Class laporan_harian
    ...
    Dim perk1, perk2, sandi1, sandi2, saldo1, saldo2 As String
    ...
    Private Sub laporan_harian_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        hapustemp()
        Me.Dispose()
    End Sub

    Private Sub laporan_harian_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.Dispose()
        End If
    End Sub

    Private Sub laporan_harian_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub report()
        ...
        If TextBox2.Text = "neraca saldo" Then
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('1') AND temp_username='" & MDIParent1.username.Text & "' ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca"))
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('2','3') AND temp_username='" & MDIParent1.username.Text & "' ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca2"))
        ElseIf TextBox2.Text = "labarugi" Then
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('4') AND temp_username='" & MDIParent1.username.Text & "' ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca"))
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('3','5','6') AND temp_username='" & MDIParent1.username.Text & "' ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca2"))
        End If
        ...
        ReportViewer1.LocalReport.DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("DataSet2", ds.Tables("temp_neraca2")))
        ...
        param(8) = New ReportParameter("perk1", perk1 + " ")
        param(9) = New ReportParameter("perk2", perk2 + " ")
        param(10) = New ReportParameter("sandi1", " " + sandi1 + " ")
        param(11) = New ReportParameter("sandi2", " " + sandi2 + " ")
        param(12) = New ReportParameter("saldo1", " " + saldo1)
        param(13) = New ReportParameter("saldo2", " " + saldo2)
    ...
    Sub data_neraca()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            If dt.Rows(i)("perkiraan_kode") = "1.190" Then
                perk1 = dt.Rows(i)("perkiraan_nama")
                sandi1 = dt.Rows(i)("perkiraan_kode").ToString.Split(".")(1)
                saldo1 = FormatNumber(saldo_akhir, 0).Replace(".", ",")
            ElseIf dt.Rows(i)("perkiraan_kode") = "1.310" Then
                perk2 = dt.Rows(i)("perkiraan_nama")
                sandi2 = dt.Rows(i)("perkiraan_kode").ToString.Split(".")(1)
                saldo2 = FormatNumber(saldo_akhir, 0).Replace(".", ",")
            Else
    ...
    Sub data_labarugi()
        da = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml FROM kode_perkiraan WHERE perkiraan_umum IN('3','4','5','6') AND perkiraan_parent IN('2','2.100','2.110','2.111','2.130','2.150','2.160','2.165','2.170','2.200','2.290','2.300')", koneksi)
        dt = New DataTable
        da.Fill(dt)
