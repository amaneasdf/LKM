...
Public Class laporan_rincian_mutasi
    ...
    Private Sub laporan_rincian_mutasi_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
    End Sub
    Private Sub laporan_rincian_mutasi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub report()
        ...
        da.SelectCommand = New MySqlCommand("SELECT * FROM temp_rincian_mutasi WHERE temp_username='" & MDIParent1.username.Text & "' ORDER BY temp_tanggal ASC", koneksi)
        da.Fill(ds.Tables("temp_rincian_mutasi"))

        ReportViewer1.ProcessingMode = ProcessingMode.Local
        ReportViewer1.LocalReport.DataSources.Clear()
        ReportViewer1.LocalReport.DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("DataSet1", ds.Tables("temp_rincian_mutasi")))
    ...
    Sub hapustemp()
        cd = New MySqlCommand("DELETE FROM temp_rincian_mutasi WHERE temp_username='" & MDIParent1.username.Text & "'", koneksi)
        cd.ExecuteNonQuery()
    End Sub
    ...
    Sub perhitunglabarugitahunberjalan()
        ...
        waktu_awal1 = Format(tanggal, "yyyy").ToString + "-01-01 00:00:00"
        If Format(tanggal, "MM dd") = "01 01" Then
            waktu_awal2 = Format(tanggal, "yyyy").ToString + "-01-01 00:00:00"
        Else
            waktu_awal2 = Format(DateAdd("d", (0 - 1), tanggal), "yyyy-MM-dd").ToString + " 23:59:59"
        End If
        waktu_dk1 = Format(tanggal, "yyyy-MM-dd").ToString + " 00:00:00"
        waktu_dk2 = Format(tanggal, "yyyy-MM-dd").ToString + " 23:59:59"
        waktu_akhir1 = Format(tanggal, "yyyy").ToString + "-01-01 00:00:00"
        waktu_akhir2 = Format(tanggal, "yyyy-MM-dd").ToString + " 23:59:59"
    ...
    Sub data()
        Dim xx As Integer = 0
        Do Until Format(DateAdd("d", xx - 1, DateTimePicker1.Value), "yyyy-MM-dd") = Format(DateTimePicker2.Value, "yyyy-MM-dd")
            tanggal = DateAdd("d", xx, DateTimePicker1.Value)
            saldo_awal = 0
            saldo_debet = 0
            saldo_kredit = 0
            saldo_akhir = 0
            ...
            tgl1 = Format(DateAdd("d", xx, DateTimePicker1.Value), "yyyy-MM-dd")
            ...
            da = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml FROM kode_perkiraan WHERE perkiraan_kode='" & TextBox2.Text & "'", koneksi)
            dt = New DataTable
            da.Fill(dt)
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                cd = New MySqlCommand("INSERT INTO temp_rincian_mutasi VALUES ('" & tgl1 & "', '" & saldo_awal & "', '" & saldo_debet & "', '" & saldo_kredit & "', '" & saldo_akhir & "', '" & MDIParent1.username.Text & "')", koneksi)
                cd.ExecuteNonQuery()
            Next
            xx = xx + 1
        Loop
    ...
    Sub pilihan()
        If perhitungan = "tahun lalu" Then
            Select Case kd
                Case "2.360"
                    If LR(84) >= 0 Then
                        LR(83) = saldo_awal + Val(LR(84))
                    End If

                Case "2.370"
                    If LR(84) < 0 Then
                        LR(82) = saldo_awal + Val(Val(LR(84)) * (0 - 1))
                    End If
            End Select
        End If
        ...
        Select Case kd
            ...
            Case "2.150" 'beban op
                ...
                LR(98) = Val(Val(LR(3)) - Val(LR(2))) - Val(Val(LR(6)) - Val(LR(7))) 'debet/kredit ops

                If LR(99) < 0 Then '-> saldo awal
                    LR(97) = LR(99) * (0 - 1) ' jika rugi ops
                Else
                    LR(97) = LR(99) 'jika laba ops
                End If

                If LR(98) < 0 Then
                    LR(96) = LR(98) * (0 - 1) 'debet laba/rugi ops
            ...
            Case "2.300" 'beban non op
                ...
                LR(93) = Val(Val(LR(11)) - Val(LR(10))) - Val(Val(LR(14)) - Val(LR(15))) 'debet/kredit non ops

                If LR(94) < 0 Then
                    LR(92) = LR(94) * (0 - 1) 'jika rugi non ops
                Else
                ...
                If LR(89) < 0 Then
                    LR(87) = LR(89) * (0 - 1) 'jika rugi tahun berjalan
                Else
                    LR(87) = LR(89) 'jika laba tahun berjalan
                End If

                If LR(88) < 0 Then
                    LR(86) = LR(88) * (0 - 1) 'debet rugi tahun berjalan
            ...
            Case "2.270" 'laba operasional
                If LR(99) > 0 Then
                    saldo_awal = saldo_awal + LR(97)
                End If
                If LR(98) > 0 Then
                    saldo_debet = saldo_debet + LR(96)
                    saldo_kredit = saldo_kredit + LR(95)
                End If
            ...
            Case "2.310" 'laba non operasional
                If LR(94) > 0 Then
                    saldo_awal = saldo_awal + LR(92)
                End If
                If LR(93) > 0 Then
                    saldo_debet = saldo_debet + LR(91)
                    saldo_kredit = saldo_kredit + LR(90)
                End If
            ...
            Case "2.330" 'laba tahun berjalan
                If LR(89) >= 0 Then
                    saldo_awal = saldo_awal + LR(87)
                End If
                If LR(88) >= 0 Then
                    saldo_debet = saldo_debet + LR(86)
                    saldo_kredit = saldo_kredit + LR(85)
                End If
            ...
            Case "2.370" 'jumlah rugi 2
                If LR(84) < 0 Then
                    saldo_awal = saldo_awal + (Val(LR(84)) * (0 - 1))
                End If

                saldo_debet = saldo_debet + Val(LR(18)) + Val(LR(26))
            ...
            Case "1.308" 'rugi tahun berjalan
                If LR(84) < 0 Then
                    saldo_awal = saldo_awal + LR(84) * (0 - 1)
                End If
                saldo_debet = saldo_debet + Val(LR(18)) + Val(LR(26))
