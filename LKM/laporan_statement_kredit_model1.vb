...
Public Class laporan_statement_kredit_model1
    Private Sub laporan_statement_kredit_model1_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
    End Sub

    Private Sub laporan_statement_kredit_model1_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.Dispose()
        End If
    End Sub
    Private Sub laporan_statement_kredit_model1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub report()
        ...
        With statement_transaksi
            param(8) = New ReportParameter("no_rekening", .tno_rekening.Text)
            param(15) = New ReportParameter("plafon_akad", .tplafon.Text)
            param(16) = New ReportParameter("suku_bunga", .tsuku_bunga.Text + " %")
            param(17) = New ReportParameter("sistem_bunga", .tsistem_bunga.Text)
            param(14) = New ReportParameter("skim_jenis_kredit", .tskim_kredit.Text)
            param(18) = New ReportParameter("kolek", .tkolek.Text + " ")
            param(19) = New ReportParameter("tgk_pokok", .ttunggakan_pokok.Text + " (" + .ttunggakan_pokok2.Text + ")")
            param(20) = New ReportParameter("tgk_bunga", .ttunggakan_bunga.Text + " (" + .ttunggakan_bunga2.Text + ")")
            param(21) = New ReportParameter("periode_pinjaman", Format(.DateTimePicker1.Value, "dd MMM yyyy") + " | " + .tjkw.Text + " | " + Format(.DateTimePicker2.Value, "dd MMM yyyy"))
            ...
            param(7) = New ReportParameter("nasabah_id", nasabah_id)
            param(9) = New ReportParameter("nama_nasabah", nama_nasabah)
            param(10) = New ReportParameter("alamat_nasabah", alamat)
            param(11) = New ReportParameter("no_hp", no_hp)
            param(12) = New ReportParameter("no_ktp", no_ktp)
            param(13) = New ReportParameter("no_tgl_akad", no_akad_awal)
            rd.Close()
            ...
            Catch ex As Exception
                kolektor = " "
            End Try
            param(23) = New ReportParameter("kolektor", kolektor)
            rd.Close()
            ...
            param(22) = New ReportParameter("agunan", agunan)
