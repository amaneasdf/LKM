...
Public Class laporan_deret_labarugi
    ...
    Dim saldo_awal, saldo_debet, saldo_kredit, saldo_akhir, jenis, kd, total_saldo As String
    ...
    Private Sub laporan_deret_labarugi_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        hapustemp()
        Me.Dispose()
    End Sub
    Private Sub laporan_deret_labarugi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub update_labarugi_tahunberjalan()
        cd = New MySqlCommand("SELECT IFNULL(SUM(deret_total), 0) AS laba_rugi_tahun_berjalan FROM temp_laporan_deret WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode IN ('2.330','2.340')", koneksi)
        rd = cd.ExecuteReader
        rd.Read()
        ...
        If laba_rugi_tahun_berjalan > 0 Then
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.340' ", koneksi)
            cd.ExecuteNonQuery()
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='" & laba_rugi_tahun_berjalan & "' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.330' ", koneksi)
            cd.ExecuteNonQuery()
        ElseIf laba_rugi_tahun_berjalan < 0 Then
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.330' ", koneksi)
            cd.ExecuteNonQuery()
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='" & laba_rugi_tahun_berjalan & "' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.340' ", koneksi)
            cd.ExecuteNonQuery()
        Else
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode IN ('2.330', '2.340')", koneksi)
            cd.ExecuteNonQuery()
        End If
    End Sub
   
    Sub update_jumlah_labarugi()
        cd = New MySqlCommand("SELECT IFNULL(SUM(deret_total), 0) AS jumlah_laba_rugi FROM temp_laporan_deret WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode IN ('2.360','2.370')", koneksi)
        rd = cd.ExecuteReader
        rd.Read()
        ...
        If jumlah_laba_rugi > 0 Then
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.370' ", koneksi)
            cd.ExecuteNonQuery()
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='" & jumlah_laba_rugi & "' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.360' ", koneksi)
            cd.ExecuteNonQuery()
        ElseIf jumlah_laba_rugi < 0 Then
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.360' ", koneksi)
            cd.ExecuteNonQuery()
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='" & jumlah_laba_rugi & "' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode = '2.370' ", koneksi)
            cd.ExecuteNonQuery()
        Else
            cd = New MySqlCommand("UPDATE temp_laporan_deret SET deret_total='0' WHERE deret_username='" & MDIParent1.username.Text & "' AND deret_perkiraan_kode IN ('2.360', '2.370')", koneksi)
            cd.ExecuteNonQuery()
        End If
    End Sub
    ...
    Public Sub report()
        ...
        If TextBox4.Text = "1" Then
            txt = ""
        Else
            txt = " AND (deret_januari <> 0 OR deret_februari <> 0 OR deret_maret <> 0 OR deret_april <> 0 OR deret_mei <> 0 OR deret_juni <> 0 OR deret_juli <> 0 OR deret_agustus <> 0 OR deret_september <> 0 OR deret_oktober <> 0 OR deret_november <> 0 OR deret_desember <> 0 OR deret_total <> 0)"
        End If
    ...
    Sub data_labarugi()
        If TextBox3.Text = "harian" Then
            daa = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_nama, perkiraan_parent, perkiraan_d_or_k FROM kode_perkiraan WHERE perkiraan_umum IN('3','4','5','6') AND perkiraan_parent IN('2','2.100','2.110','2.111','2.130','2.150','2.160','2.165','2.170','2.200','2.290','2.300')", koneksi)
        ElseIf TextBox3.Text = "skontro" Then
            daa = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_nama, perkiraan_parent, perkiraan_d_or_k FROM kode_perkiraan WHERE perkiraan_umum IN('4','5','6')", koneksi)
        End If
        ...
        For ii As Integer = 0 To dtt.Rows.Count - 1
            ...
            For b As Integer = 1 To Format(DateTimePicker2.Value, "MM")
                ...
                saldo_akhir = 0
                'End If
                'tgl1 = Format(tglakhirbulan, "yyyy-MM").ToString + "-01 00:00:00"
                ...
                waktu_dk1 = Format(tglakhirbulan, "yyyy-MM").ToString + "-01 00:00:00"
                waktu_dk2 = Format(tglakhirbulan, "yyyy-MM-dd").ToString + " 23:59:59"
                waktu_akhir1 = Format(tglakhirbulan, "yyyy-MM").ToString + "-01 00:00:00"
                waktu_akhir2 = Format(tglakhirbulan, "yyyy-MM-dd").ToString + " 23:59:59"
                ...
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    'Else
                    If dt.Rows(i)("jml") > 0 Then
                        da1 = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal1, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet1, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit1, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir1, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml1 FROM kode_perkiraan WHERE perkiraan_parent='" & dt.Rows(i)("perkiraan_kode") & "'", koneksi)
                        dt1 = New DataTable
                        da1.Fill(dt1)
                        For i1 As Integer = 0 To dt1.Rows.Count - 1
                            saldo_awal = saldo_awal + dt1.Rows(i1)("saldoawal1")
                            ...
                            'Else
                            If dt1.Rows(i1)("jml1") > 0 Then
                                da2 = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal2, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet2, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit2, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir2, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml2 FROM kode_perkiraan WHERE perkiraan_parent='" & dt1.Rows(i1)("perkiraan_kode") & "'", koneksi)
                                dt2 = New DataTable
                                da2.Fill(dt2)
                                For i2 As Integer = 0 To dt2.Rows.Count - 1
                                    saldo_awal = saldo_awal + dt2.Rows(i2)("saldoawal2")
                                    ...
                                    'Else
                                    If dt2.Rows(i2)("jml2") > 0 Then
                                        da3 = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal3, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet3, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit3, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir3, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml3 FROM kode_perkiraan WHERE perkiraan_parent='" & dt2.Rows(i2)("perkiraan_kode") & "'", koneksi)
                                        dt3 = New DataTable
                                        da3.Fill(dt3)
                                        For i3 As Integer = 0 To dt3.Rows.Count - 1
                                            saldo_awal = saldo_awal + dt3.Rows(i3)("saldoawal3")
                                            ...
                                            'Else
                                            If dt3.Rows(i3)("jml3") > 0 Then
                                                da4 = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal4, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet4, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit4, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir4, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml4 FROM kode_perkiraan WHERE perkiraan_parent='" & dt3.Rows(i3)("perkiraan_kode") & "'", koneksi)
                                                dt4 = New DataTable
                                                da4.Fill(dt4)
                                                For i4 As Integer = 0 To dt4.Rows.Count - 1
                                                    saldo_awal = saldo_awal + dt4.Rows(i4)("saldoawal4")
                                                    ...
                                                    'Else
                                                    If dt4.Rows(i4)("jml4") > 0 Then
                                                        da5 = New MySqlDataAdapter("SELECT perkiraan_kode, perkiraan_umum, perkiraan_type, perkiraan_parent, perkiraan_nama, perkiraan_d_or_k, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_awal1 & "','" & waktu_awal2 & "') AS saldoawal5, hitung_saldo_perkiraan_debet_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldodebet5, hitung_saldo_perkiraan_kredit_per_tanggal(perkiraan_kode,'" & waktu_dk1 & "','" & waktu_dk2 & "') AS saldokredit5, hitung_saldo_perkiraan_akhir_per_tanggal(perkiraan_kode,'" & waktu_akhir1 & "','" & waktu_akhir2 & "') AS saldoakhir5, hitung_jumlah_perkiraan_anak(perkiraan_kode) AS jml5 FROM kode_perkiraan WHERE perkiraan_parent='" & dt4.Rows(i4)("perkiraan_kode") & "'", koneksi)
                                                        dt5 = New DataTable
                                                        da5.Fill(dt5)
                                                        For i5 As Integer = 0 To dt5.Rows.Count - 1
                                                            saldo_awal = saldo_awal + dt5.Rows(i5)("saldoawal5")
                ...
                End If
                bulan(b) = saldo_akhir
                total_saldo = Val(total_saldo) + Val(saldo_akhir)
            Next
            ...
            cd = New MySqlCommand("INSERT INTO temp_laporan_deret VALUES ('" & ii + 1 & "','" & dtt.Rows(ii)("perkiraan_kode") & "', '" & dtt.Rows(ii)("perkiraan_nama") & "', '" & dtt.Rows(ii)("perkiraan_kode").ToString.Split(".")(1) & "' " & gabunganbulan & " , '" & total_saldo & "', '" & jenis & "','" & MDIParent1.username.Text & "')", koneksi)
            cd.ExecuteNonQuery()
