...
Public Class laporan_skontro
    Public da1, da2, da3, da4, da5 As MySqlDataAdapter
    Public dt1, dt2, dt3, dt4, dt5 As DataTable

    Dim perk1, perk2, saldo1, saldo2, saldo_awal, saldo_debet, saldo_kredit, saldo_akhir, jenis, kd As String
    ...
    Private Sub laporan_skontro_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        hapustemp()
        Me.Dispose()
    End Sub

    Private Sub laporan_skontro_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.Dispose()
        End If
    End Sub
    Private Sub laporan_skontro_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub report()
        ...
        If TextBox2.Text = "neraca saldo" Then
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('1') AND temp_username='" & MDIParent1.username.Text & "' " & txt & " ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca"))
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('2','3') AND temp_username='" & MDIParent1.username.Text & "' " & txt & " ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca2"))
        ElseIf TextBox2.Text = "labarugi" Then
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('4') AND temp_username='" & MDIParent1.username.Text & "' " & txt & " ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca"))
            da.SelectCommand = New MySqlCommand("SELECT * FROM temp_neraca WHERE temp_perkiraan_umum IN('3','5','6') AND temp_username='" & MDIParent1.username.Text & "' " & txt & " ORDER BY temp_perkiraan_kode ASC", koneksi)
            da.Fill(ds.Tables("temp_neraca2"))
        End If
        ...
        param(10) = New ReportParameter("saldo1", " " + saldo1)
        param(11) = New ReportParameter("saldo2", " " + saldo2)
